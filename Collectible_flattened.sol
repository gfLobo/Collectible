...
interface ICollectible {
    ...
    event MembershipTermsUpdated(uint256 membershipSignatureFee);

    // Function to get minter Membership signature
    function getMembershipSignature() external payable;
    ...
    // Function to update the minter Membership signature fee
    function updateMembershipSignaturePrice(uint256 _membershipSignatureFee) external;
...
/// @author gfLobo
/// @notice This contract implements a collectible system based on ERC721 with guild-based memberships.
    ...
    ReentrancyGuard
{
    ...
    /// @notice Role identifier for members of the system.
    ...
    /// @notice Fee required for obtaining a membership signature, in ETH.
    uint256 public membershipSignatureFee;
    ...
    /// @param _membershipSignatureFee The fee required to become a member.
    constructor(
        string memory _tokenName,
        uint256 _mintBaseFee,
        uint256 _mintRateIncrementPercentage,
        uint256 _membershipSignatureFee
    )
        ERC721(_tokenName, "CERC721")
    {
        ...
        updateMembershipSignaturePrice(_membershipSignatureFee);
    }
    ...
    /// @notice Allows users to donate ETH to the contract and receive the contributor role.
    function donate() public payable override {
        _grantRole(CONTRIBUTOR_ROLE, msg.sender);
    }

    /// @notice Allows a user to acquire the membership signature by paying the membership fee.
    /// @dev The membership fee is set by the admin.
    function getMembershipSignature()
        public
        payable
        override
        onlyIfNotPaused
    {
        require(msg.value == membershipSignatureFee, "Not enough ETH!");
        _grantRole(MEMBER_ROLE, msg.sender);
    }
    ...
    /// @notice Updates the membership signature fee.
    /// @param _membershipSignatureFee The new fee for membership signatures.
    /// @dev Only the admin can call this function.
    function updateMembershipSignaturePrice(uint256 _membershipSignatureFee)
        public
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        membershipSignatureFee = _membershipSignatureFee;
        emit MembershipTermsUpdated(_membershipSignatureFee);
    }
